...
def basic_block(x, filters, kernel_size=3, stride=1,
                conv_shortcut=True, global_bn=True, name=None):
    ...
    global_bn: default True, Normalize and scale inputs or activations
    synchronously across replicas if True
    ...
    if conv_shortcut:
        ...
        if global_bn:
            shortcut = tfkl.experimental.SyncBatchNormalization(
                axis=bn_axis, momentum=0.9,
...
def bottleneck(x, filters, kernel_size=3, stride=1,
               conv_shortcut=True, global_bn=True, name=None):
...
def basic_stack(x, filters, blocks, stride1=2, global_bn=True, name=None):
    ...
    """
    x = basic_block(
        x, filters, stride=stride1, global_bn=global_bn, name=name + '_block1')

    for i in range(2, blocks + 1):
        x = basic_block(
            x, filters, conv_shortcut=False,
            global_bn=global_bn, name=name + '_block' + str(i))
    return x


def bottleneck_stack(x, filters, blocks, stride1=2, global_bn=True, name=None):
...
def ResNet(input_shape,
    ...
    if height <= 32 or width <= 32:
        ...
        if global_bn:
            x = tfkl.experimental.SyncBatchNormalization(
                axis=bn_axis, momentum=0.9,
                epsilon=1.001e-5, name='conv1_bn')(x)
        else:
            x = tfkl.BatchNormalization(
                axis=bn_axis, momentum=0.9,
                epsilon=1.001e-5, name='conv1_bn')(x)
