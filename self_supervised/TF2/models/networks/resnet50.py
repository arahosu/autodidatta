...
def ResNet50(include_top,
    ...
    x = ZeroPadding(padding=tuple(3*i for i in unit_vector),
                    name='conv1_pad')(img_input)
    x = Conv(64, tuple(7*i for i in unit_vector),
             strides=tuple(2*i for i in unit_vector),
             padding='valid',
             kernel_initializer='he_normal',
             name='conv1')(x)
    x = tfkl.BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = tfkl.Activation('relu')(x)
    x = ZeroPadding(padding=unit_vector, name='pool1_pad')(x)
    x = MaxPooling(tuple(3*i for i in unit_vector), strides=tuple(2*i for i in unit_vector))(x)

    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=unit_vector)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b')
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c')

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', strides=unit_vector)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b')
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c')
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='d')

    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', strides=unit_vector)
    ...
    x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', strides=unit_vector)
